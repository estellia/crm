...
namespace JIT.TestUtility.TestPay.Web.UnionPay.Wap
{
    ...
    /// 交易通知
    /// <remarks>
    /// <para>当消费者在支付页面完成支付过程后,支付前置将向商户平台发送交易通知,商户平台在交易通知中完成对订单状态的更新等处理</para>
    /// </remarks>
    /// </summary>
        ...
        public void ProcessRequest(HttpContext context)
        {
            using (StreamReader sr = new StreamReader(context.Request.InputStream))
            {
                try
                {
                    ...
                    Loggers.Debug(new DebugLogInfo() { Message = "[Wap]Encrypted Transaction Notification Request=" + strReq });
                    //解析交易通知密文,获得交易通知请求的内容
                    var req = WapGateway.ParseTransactionNotificationRequest(ConfigurationManager.AppSettings["WAPDecryptCertificateFilePath"], strReq);
                    Loggers.Debug(new DebugLogInfo() { Message = "[Wap]Decrypted Transaction Notification Request=" + req.GetContent() });
                    if (req.IsPayOK)
                    {//用户支付成功
                        try
                        {
                            ...
                            Loggers.Debug(new DebugLogInfo() { Message = string.Format("[Wap]Transaction Notification Response={0}", rsp) });
