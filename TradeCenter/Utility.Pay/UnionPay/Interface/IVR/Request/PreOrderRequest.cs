...
namespace JIT.Utility.Pay.UnionPay.Interface.IVR.Request
{
    ...
    public class PreOrderRequest : BaseAPIRequest
    {
        ...
        public PreOrderRequest()
        {
            ...
            this.GatewayType = GatewayTypes.IVR;
        }
        #endregion
        ...
        /// 交易类型.如不填写，默认为消费
        /// <remarks>
        /// <para>预订单时可选择的交易类型有：ExpressConsumption和PreAuthorization以及Authentication</para>
        /// </remarks>
        /// </summary>
        public IVRTransTypes? TransType
        {
            get { return this.GetParameter<IVRTransTypes?>("transType"); }
        ...
        /// </remarks>
        /// </summary>
        public IVRModes? Mode
        {
            get { return this.GetParameter<IVRModes?>("msgExt"); }
        ...
        public override string GetContent()
        {
            ...
            {
                if(this.MerchantOrderID.Length>30)
                    throw new ArgumentException("参数MerchantOrderID的长度不能超过30.");
            ...
            {
                sb.AppendFormat("<merchantOrderTime>{0}</merchantOrderTime>", this.MerchantOrderTime.Value.ToString("yyyy-MM-dd HH:mm:ss"));
            }
            else
            {
            ...
            if (this.MerchantOrderAmt != null)
            {
                ...
                //测试环境：锁死订单金额为1分
                //sb.AppendFormat("<merchantOrderAmt>{0}</merchantOrderAmt>", 1);
            }
            else
            {
            ...
            if (this.TransTimeout != null)
            {
                ...
                //sb.AppendFormat("<transTimeout>{0}</transTimeout>", this.TransTimeout.Value.ToString("yyyy-MM-dd HH:mm:ss"));
            }
            ...
            if (this.Mode != null)
            {
                sb.AppendFormat("<msgExt>{0}</msgExt>", this.Mode.Value.GetCode());
            }
            else
            {
                throw new ArgumentException("参数Mode为必填参数.");
            }
            if (this.Misc != null)
            {
