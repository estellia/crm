...
using ChainClouds.Weixin.MP.AppStore;
...
namespace ChainClouds.Weixin.MP.MessageHandlers
{
    /// <summary>
    /// 微信请求的集中处理方法
    /// 此方法中所有过程，都基于ChainClouds.Weixin.MP的基础功能，只为简化代码而设。
    /// </summary>
        ...
        where TC : class, IMessageContext<IRequestMessageBase, IResponseMessageBase>, new()
    {
        ...
        /// 这里是ChainClouds.Weixin.MP，根据请求消息半段在ResponseDocument基础上是否需要再次进行加密（每次获取重新加密，所以结果会不同）
        /// </summary>
        ...
        /// <param name="developerInfo">连锁掌柜开发者信息，如果不为空，则优先请求云端应用商店的资源</param>
        ...
        public override void Execute()
        {
            ...
            try
            {
                ...
                //ResponseMessage = this.TextResponseMessage;
                //记录上下文
