...
namespace ChainClouds.Weixin.MP.Agent
{
    /// <summary>
    /// 代理请求
    ...
    public static class MessageAgent
    {
        ...
        /// <param name="useChainCloudsKey">是否使用ChainCloudsKey，如果使用，则token为ChainCloudsKey</param>
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static string RequestXml(this IMessageHandler messageHandler, string url, string token, Stream stream, bool useChainCloudsKey = false, int timeOut = AGENT_TIME_OUT)
        {
        ...
        /// <summary>
        /// 对接（P2P）平台，获取Xml结果，使用ChainCloudsKey对接
        /// ChainCloudsKey的获取方式请看：
        /// </summary>
        /// <param name="messageHandler"></param>
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static string RequestChainCloudsXml(this IMessageHandler messageHandler, string chaincloudsKey, string xml, string chaincloudsDomainName = "www.chainclouds.com", int timeOut = AGENT_TIME_OUT)
        {
            if (messageHandler != null)
            {
                messageHandler.UsedMessageAgent = true;
            }
            var url = "http://" + chaincloudsDomainName + "/App/Weixin?chaincloudsKey=" + chaincloudsKey;//官方地址
            using (MemoryStream ms = new MemoryStream())
            {
                //这里用ms模拟Request.InputStream
                using (StreamWriter sw = new StreamWriter(ms))
                {
                    ...
                    sw.Flush();
                    sw.BaseStream.Position = 0;
                    return messageHandler.RequestXml(url, chaincloudsKey, sw.BaseStream, timeOut: timeOut);
                }
            }
        }
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static IResponseMessageBase RequestChaincloudsResponseMessage(this IMessageHandler messageHandler, string chaincloudsKey, string xml, string chaincloudsDomainName = "www.chainclouds.com", int timeOut = AGENT_TIME_OUT)
        {
            return messageHandler.RequestChainCloudsXml(chaincloudsKey, xml, chaincloudsDomainName, timeOut).CreateResponseMessage();
        }
        ...
        /// <param name="timeOut">代理请求超时时间（毫秒）</param>
        /// <returns></returns>
        public static IResponseMessageBase RequestChaincloudsResponseMessage(this IMessageHandler messageHandler, string chaincloudsKey, XDocument document, string chaincloudsDomainName = "www.chainclouds.com", int timeOut = AGENT_TIME_OUT)
        {
            return messageHandler.RequestChainCloudsXml(chaincloudsKey, document.ToString(), chaincloudsDomainName, timeOut).CreateResponseMessage();
        }
