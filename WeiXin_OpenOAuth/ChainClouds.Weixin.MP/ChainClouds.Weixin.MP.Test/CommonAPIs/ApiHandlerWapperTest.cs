...
namespace ChainClouds.Weixin.MP.Test.CommonAPIs
{
    [TestClass]
    public class ApiHandlerWapperTest : CommonApiTest
    {
        ...
        public void TryCommonApiTest()
        {
            //正确的测试
            {
                var result = ApiHandlerWapper.TryCommonApi(ChainClouds.Weixin.MP.CommonAPIs.CommonApi.GetMenu, base._appId);
            ...
            //忽略appId
            {
                var result = ApiHandlerWapper.TryCommonApi(ChainClouds.Weixin.MP.CommonAPIs.CommonApi.GetMenu);
                Assert.IsTrue(result.menu.button.Count > 0);
                Console.WriteLine(result.menu.button.Count);
            }
            ...
            //错误的AccessToken
            {
                ...
                var result = ApiHandlerWapper.TryCommonApi(ChainClouds.Weixin.MP.CommonAPIs.CommonApi.GetMenu, "12345678901234567890这是错误的AccessToken");
                Console.WriteLine("当前AccessToken：" + accessToken);
