...
chainclouds.menu = {
    ...
    init: function () {
        ...
        $("#buttonDetails_type").change(chainclouds.menu.typeChanged);
            ...
            $.getJSON('/Menu/GetToken?t=' + Math.random(), { appId: $('#menuLogin_AppId').val(), appSecret: $('#menuLogin_AppSecret').val() },
                function (json) {
                    if (json.access_token) {
                        chainclouds.menu.setToken(json.access_token);
                    } else {
        ...
        $('#menuLogin_SubmitOldToken').click(function () {
            chainclouds.menu.setToken($('#menuLogin_OldToken').val());
        });

        $('#btnGetMenu').click(function () {
            menuState.html('获取菜单中...');
            $.getJSON('/Menu/GetMenu?t=' + Math.random(), { token: chainclouds.menu.token }, function (json) {
                if (json.menu) {
        ...
        $('#btnDeleteMenu').click(function () {
            ...
            $.getJSON('/Menu/DeleteMenu?t=' + Math.random(), { token: chainclouds.menu.token }, function (json) {
                if (json.Success) {
