...
ChainClouds.Weixin.dll：所有ChainClouds.Weixin.[xx].dll的基础库

ChainClouds.Weixin.MP.dll：ChainClouds.Weixin.MP 微信SDK核心库

ChainClouds.Weixin.MP.MvcExtension.dll：在ChainClouds.Weixin.MP.dll基础上，为MVC4.0项目提供的扩展库，简化判断和输出。如果使用Webforms或Wcf等方式与维新服务器通讯，可以忽略这个dll。

ChainClouds.Weixin.QY.dll：微信企业号SDK

ChainClouds.Weixin.Open.dll：微信开放平台SDK


ChainClouds.Weixin.MP.MvcExtension更新日志见：https://github.com/JeffreySu/WeiXinMPSDK/blob/master/ChainClouds.Weixin.MP.MvcExtension/readme.md


ChainClouds.Weixin.MP.dll升级记录
----------
...
添加消息上下文删除事件，Demo见/ChainClouds.Weixin.MP.Web/ChainClouds.Weixin.MP.Web.CommonService/CustomMessageHandler/CustomMessageContext.cs
...
添加ChainClouds.Weixin.MP.HttpUtility.RequestUtility.IsWeixinClientRequest()方法，用于判断请求是否发起自微信客户端的浏览器。
