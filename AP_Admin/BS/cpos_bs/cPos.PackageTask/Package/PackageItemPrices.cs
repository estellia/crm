...
{
    public class PackageItemPrices
    {
            ...
            string batId, string customerId, string unitId, string userId)
        {
            ...
            string methodKey = "Task.PackageItemPrices";
            string ifCode = TaskCode.T007.ToString();
            ...
            try
            {
                ...
                var dataService = new ExchangeBsService.ItemPriceBsService();

                // 获取总数量
                int count = dataService.GetItemPriceNotPackagedCount(customerId, userId, unitId);
                if (count <= 0)
                {
                ...
                // 创建数据包
                string pkgTypeCode = PackageTypeMethod.ITEMPRICES.ToString();
                ...
                while (num <= count)
                {
                    IList<cPos.Model.ItemPriceInfo> items = dataService.GetItemPriceListPackaged(
                        customerId, userId, unitId, num, pageSize);
                    if (items.Count <= 0) break;
                    num += pageSize;

                    Hashtable htPkgf = pkgService.CreateItemPricesPackageFile(
                    ...
                    // 记录数据打包批次号
                    dataService.SetItemPriceBatInfo(customerId, userId, unitId, batId, items);
                }
                ...
                // 更新数据打包标识
                dataService.SetItemPriceIfFlagInfo(customerId, userId, unitId, batId);
