...
namespace cPos.PackageTask
{
    public class PackageUnits
    {
            ...
            string batId, string customerId, string unitId, string userId)
        {
            ...
            string methodKey = "Task.PackageUnits";
            string ifCode = TaskCode.T004.ToString();
            ...
            try
            {
                ...
                // 获取总数量
                int count = dataService.GetUnitNotPackagedCount(customerId, userId, unitId);
                if (count <= 0)
                {
                ...
                // 创建数据包
                string pkgTypeCode = PackageTypeMethod.UNITS.ToString();
                ...
                while (num <= count)
                {
                    IList<cPos.Model.UnitInfo> items = dataService.GetUnitListPackaged(
                    ...
                    {
                        item.UnitPropertyInfoList = dataService.GetUnitPropInfoListPackaged(
                            customerId, userId, unitId, item.Id);
                        item.WarehouseInfoList = dataService.GetWarehouseByUnitIDPackaged(
                            customerId, userId, unitId, item.Id);
                    }

                    Hashtable htPkgf = pkgService.CreateUnitsPackageFile(
                    ...
                    // 记录数据打包批次号
                    dataService.SetUnitBatInfo(customerId, userId, unitId, batId, items);
                }
                ...
                // 更新数据打包标识
                dataService.SetUnitIfFlagInfo(customerId, userId, unitId, batId);
