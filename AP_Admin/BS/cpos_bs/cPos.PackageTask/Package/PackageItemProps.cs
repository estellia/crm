...
namespace cPos.PackageTask
{
    public class PackageItemProps
    {
            ...
            string batId, string customerId, string unitId, string userId)
        {
            ...
            string methodKey = "Task.PackageItemProps";
            string ifCode = TaskCode.T010.ToString();
            ...
            try
            {
                ...
                var dataService = new ExchangeBsService.ItemAuthService();

                string pkgTypeCode = PackageTypeMethod.ITEMPROPS.ToString();
                ...
                // 获取数据
                var items = dataService.GetPropByItemPackaged(customerId, userId, unitId);
                ...
                // 生成数据包文件
                Hashtable htPkgf = pkgService.CreateItemPropDefsPackageFile(
