...
namespace cPos.PackageTask
{
    public class PackageItems
    {
            ...
            string batId, string customerId, string unitId, string userId)
        {
            ...
            string methodKey = "Task.PackageItems";
            string ifCode = TaskCode.T002.ToString();
            ...
            try
            {
                ...
                var dataService = new ExchangeBsService.ItemAuthService();//在这里引用调用了数据库实体

                // 获取总数量
                int count = dataService.GetItemNotPackagedCount(customerId, userId, unitId);//获取未打包的商品数量
                if (count <= 0)
                {
                ...
                // 创建数据包
                string pkgTypeCode = PackageTypeMethod.ITEMS.ToString();
                ...
                {
                    //使用实体类
                    IList<cPos.Model.ItemInfo> items = dataService.GetItemListPackaged(
                    ...
                    Hashtable htPkgf = pkgService.CreateItemsPackageFile(
                    ...
                    // 记录数据打包批次号
                    dataService.SetItemBatInfo(customerId, userId, unitId, batId, items);
                }
                ...
                // 更新数据打包标识
                dataService.SetItemIfFlagInfo(customerId, userId, unitId, batId);
