...
namespace cPos.PackageTask
{
    public class PackageSkuPrices
    {
            ...
            string batId, string customerId, string unitId, string userId)
        {
            ...
            string methodKey = "Task.PackageSkuPrices";
            string ifCode = TaskCode.T008.ToString();
            ...
            try
            {
                ...
                var dataService = new ExchangeBsService.SkuPriceBsService();

                // 获取总数量
                int count = dataService.GetSkuPriceNotPackagedCount(customerId, userId, unitId);
                if (count <= 0)
                {
                ...
                // 创建数据包
                string pkgTypeCode = PackageTypeMethod.SKUPRICES.ToString();
                ...
                while (num <= count)
                {
                    IList<cPos.Model.SkuPriceInfo> items = dataService.GetSkuPriceListPackaged(
                        customerId, userId, unitId, num, pageSize);
                    if (items.Count <= 0) break;
                    num += pageSize;

                    Hashtable htPkgf = pkgService.CreateSkuPricesPackageFile(
                    ...
                    // 记录数据打包批次号
                    dataService.SetSkuPriceBatInfo(customerId, userId, unitId, batId, items);
                }
                ...
                // 更新数据打包标识
                dataService.SetSkuPriceIfFlagInfo(customerId, userId, unitId, batId);
