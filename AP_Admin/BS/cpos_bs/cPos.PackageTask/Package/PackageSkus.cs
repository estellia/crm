...
namespace cPos.PackageTask
{
    public class PackageSkus
    {
            ...
            string batId, string customerId, string unitId, string userId)
        {
            ...
            string methodKey = "Task.PackageSkus";
            string ifCode = TaskCode.T003.ToString();
            ...
            try
            {
                ...
                // 获取总数量
                int count = dataService.GetSkuNotPackagedCount(customerId, userId, unitId);
                if (count <= 0)
                {
                ...
                // 创建数据包
                string pkgTypeCode = PackageTypeMethod.SKUS.ToString();
                ...
                while (num <= count)
                {
                    IList<cPos.Model.SkuInfo> items = dataService.GetSkuListPackaged(
                        customerId, userId, unitId, num, pageSize);
                    if (items.Count <= 0) break;
                    num += pageSize;

                    Hashtable htPkgf = pkgService.CreateSkusPackageFile(
                    ...
                    // 记录数据打包批次号
                    dataService.SetSkuBatInfo(customerId, userId, unitId, batId, items);
                }
                ...
                // 更新数据打包标识
                dataService.SetSkuIfFlagInfo(customerId, userId, unitId, batId);
