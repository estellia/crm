<%@ Page Title="" Language="C#" MasterPageFile="~/common/Site.master" AutoEventWireup="true"
    CodeFile="inout_bill_in_show.aspx.cs" Inherits="inout_bill_in_show" %>
...
<script type="text/javascript">
    ...
    function buildItemTr(item, index) {
        ...
        var tr = $("<tr class=\"" + _class + "\"><td>" + (item.item_code==null?"&nbsp;":item.item_code) + "</td><td>" + (item.item_name==null?"&nbsp;":item.item_name) + "</td><td " + getClass(1) + ">" + (item.prop_1_detail_name==null?"&nbsp;":item.prop_1_detail_name) + "</td><td " + getClass(2) + ">" + (item.prop_2_detail_name==null?"&nbsp;":item.prop_2_detail_name) + "</td><td " + getClass(3) + ">" + (item.prop_3_detail_name==null?"&nbsp;":item.prop_3_detail_name) + "</td><td " + getClass(4) + ">" + (item.prop_4_detail_name==null?"&nbsp;":item.prop_4_detail_name) + "</td><td " + getClass(5) + ">" + (item.prop_5_detail_name==null?"&nbsp;":item.prop_5_detail_name) + "</td><td>" + item.order_qty + "</td><td>" + item.enter_qty + "</td><td>" + item.std_price + "</td><td>" + item.retail_price + "</td><td>" + item.retail_amount + "</td><td><a href='#' " + show + " detail_id=\"" + item.order_detail_id + "\" index=\"" + index + "\" onclick=\"removeThis(this);\"><img src=\"../img/delete.png\" alt=\"\" title=\"删除\" /></a></td></tr>");
        return tr;
    }
    ...
    function showItem() {
        var value = openWindow("inout_bill_in_show_item.aspx", 420, 720,'');
        if (value) {
    ...
    $(function () {
        buildItem();
        $("#<%=selPurchaseUnit.ClientID %>")[0].onchanged = unitChange;
    });
    function unitChange() {
    ...
    <table border="0" cellspacing="0" cellpadding="0" class="con_tab" style="margin-left:auto; margin-right:auto" id="tab">
        <tr>
            ...
            <td width="320">
                ...
                <asp:TextBox ID="tbOrderNo" ReadOnly="true" runat="server" MaxLength="30"></asp:TextBox>
            </td>
            <td  class="td_co">
                入库单类型：
            </td>
            <td>
                ...
                <asp:ListItem Selected="False" Text="---请选择---" Value="-1"></asp:ListItem>
        ...
        <tr>
            ...
            <td class="td_lp">
                ...
                <asp:TextBox ID="tbTotalAmount" CssClass="ban_in_oper_time" ReadOnly="true"  runat="server" MaxLength="200"></asp:TextBox>
            </td>
        </tr>
        <tr>
            <td  class="td_co">
            ...
            <td class="td_lp" colspan="3">
                ...
                <asp:TextBox TextMode="MultiLine" ID="tbRemark" MaxLength="180" Height="50px" runat="server" Width="100%"></asp:TextBox>
                </div>
            </td>
        </tr>
    </table>
        ...
        <table cellpadding="0" cellspacing="1" class="ss_jg" border="0">
            <thead>
                <tr class="b_c3">
                    ...
                    <% for (int i = 1; i <= 5; i++)
                       {
                           ...
                           if (item != null)
                           {%>
                    <th scope="col" width="80" align="center" index="<%= item.display_index %>">
                        <%=item.prop_name%>
                    </th>
                    <%}
                        else
                    ...
                    <th scope="col" align="center" width="80">
                        预订数
                    </th>
                    <th scope="col" align="center" width="80">
                        入库数
