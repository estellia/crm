<%@ Page Title="" Language="C#" MasterPageFile="~/common/Site.master" AutoEventWireup="true"
    CodeFile="adjust_bill_query.aspx.cs" Inherits="inventory_adjust_bill_query" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="Server">
    <script type="text/javascript">
        ...
        function go_Modify(sender) {
            ...
            this.location.href = "adjust_bill_show.aspx?strDo=Modify&order_id=" + order_id + "&from=" + getfrom();
        }
        //转到后台查看页面 传递当前ID参数
        function go_Visible(sender) {
            this.location.href = "adjust_bill_show.aspx?strDo=Visible&order_id=" + sender + "&from=" + getfrom();
        }
        //转到添加页面
        function go_Add() {
            this.location.href = "adjust_bill_show.aspx?strDo=Create&from=" + getfrom();
        }
        $(function () {
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
    ...
    <table border="0" cellspacing="0" cellpadding="0" class="ss_tj" >
        ...
        <tr>
            ...
            <td class="td_lp" colspan="3">
                <asp:DropDownList  runat="server" ID="SelectDataFrom">
                </asp:DropDownList>
            </td>
            <td class="tit_c" >
                来源单据号：
    ...
    <div class="ss_bg">
        <asp:GridView ID="gvAdjBill" runat="server" CellPadding="0" CellSpacing="1" CssClass="ss_jg"
            AutoGenerateColumns="False" GridLines="None" ShowHeaderWhenEmpty="True" OnRowCommand="gvAdjBill_RowCommand"
            OnRowDataBound="gvAdjBill_RowDataBound">
            ...
            <Columns>
                ...
                <asp:TemplateField HeaderText="调整单单号" ItemStyle-CssClass="gv-4word">
                    <ItemTemplate>
                ...
                <asp:BoundField DataField="unit_name" HeaderText="调整单位" ItemStyle-HorizontalAlign="Center"
                ...
                <asp:BoundField DataField="order_reason_name" HeaderText="调整单类型" ItemStyle-Width="60px"
                    ItemStyle-HorizontalAlign="Center" />
                <asp:BoundField DataField="order_date" HeaderText="调整日期" ItemStyle-Width="60px" ItemStyle-HorizontalAlign="Center" />
                <asp:BoundField DataField="total_qty" HeaderText="总数" ItemStyle-Width="60px" ItemStyle-HorizontalAlign="Center" />
                <asp:BoundField DataField="status_desc" HeaderText="状态" ItemStyle-Width="60px" ItemStyle-HorizontalAlign="Center" />
                <asp:TemplateField HeaderText="操作" ItemStyle-CssClass="gv-2icon" ItemStyle-Width="125px">
                    <ItemTemplate>
                        ...
                        <asp:LinkButton OnClientClick="return confirm('确认要审批该调整单？','询问');" runat="server"
                            ID="check" CommandArgument='<%#Eval("order_id") %>' CommandName="Check"><img src="../img/approve.png" title="审批" alt="审批" /></asp:LinkButton>
                        <asp:LinkButton OnClientClick="return confirm('确认要回退该调整单？','询问');" runat="server"
                            ID="back" CommandArgument='<%#Eval("order_id") %>' CommandName="Back">
                            <img src="../img/reject.png" title="回退" alt="回退" /></asp:LinkButton>
                        <asp:LinkButton OnClientClick="return confirm('确实要删除该调整单？','询问');" runat="server"
                            ID="delete" CommandArgument='<%#Eval("order_id") %>' CommandName="ajDelete"><img src="../img/delete.png" title="删除" alt="删除" /></asp:LinkButton>
                    </ItemTemplate>
                </asp:TemplateField>
            </Columns>
