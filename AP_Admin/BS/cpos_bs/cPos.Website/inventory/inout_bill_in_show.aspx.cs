...
public partial class inout_bill_in_show : PageBase
{
    ...
    protected void btnSaveClick(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (rult)
            {
                this.Redirect("保存成功", InfoType.Info, this.Request.QueryString["from"] ?? "inout_bill_in_query.aspx");
            }
            else
            {
        ...
        catch (Exception ex)
        {
            ...
            this.InfoBox.ShowPopError(string.Format("出错了，详细内容：{0}", "系统错误，可能是入库单号已存在!"));
        }
    ...
    {
        LoadWareHouse(this.selPurchaseUnit.SelectedValue); 
    ...
    private void initInvtoryNo()
    {
        try
        {
            ...
            this.tbOrderNo.Text = bs.GetNo(loggingSessionInfo, "RO");
        }
        catch (Exception ex)
        {
    ...
    private void LoadInOutData()
    {
        try
        {
            ...
            LoadWareHouse(rult.purchase_unit_id);
    ...
    private InoutInfo GetDataFromUI()
    {
        ...
        inoutInfo.order_type_id = "C1D407738E1143648BC7980468A399B8";
    ...
    private void loadReasonType()
    {
        try
        {
            var ddlList = new cPos.Service.OrderReasonTypeService().GetOrderReasonTypeListByOrderTypeCode(loggingSessionInfo, "RO.ReasonType");
