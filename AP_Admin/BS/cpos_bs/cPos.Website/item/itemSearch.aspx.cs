...
public partial class item_itemSearch : PageBase
{
    ...
    private void GetSkuList()
    {
        try
        {
            ...
            var source = new cPos.Service.SkuService().GetSkuListByItemId(loggingSessionInfo, item_id);
    ...
    private IList<cPos.Model.ItemInfo> GetMachItemInfo()
    {
        try
        {
            return new cPos.Service.ItemService().GetItemListLikeItemName(loggingSessionInfo, this.Request["Key"]);
        }
        catch (Exception ex)
        {
            PageLog.Current.Write(ex);
