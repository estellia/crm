...
namespace cPos.Admin.Service
{
    ...
    public class ItemPropService:BaseService
    {
        ...
        public bool SetItemPropInfo(LoggingSessionInfo loggingSessionInfo, ItemInfo itemInfo)
        {
            try
            {
                if (itemInfo.ItemPropList != null)
                {
                    ...
                    MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).Update("ItemProp.InsertOrUpdate", itemInfo);
                }
                return true;
            }
            catch (Exception ex)
        ...
        {
            return MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).QueryForList<PropByItemInfo>("ItemProp.SelectItemProp", "");
