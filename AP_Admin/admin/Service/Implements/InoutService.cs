...
namespace cPos.Admin.Service
{
    ...
    public class InoutService : BaseService
    {
        ...
        /// inout 单据导入
        /// </summary>
        /// <param name="orders">inout model</param>
        /// <param name="IsTrans">是否批处理</param>
        /// <returns>Hashtable: 
        ///  status(成功：true, 失败：false)
        ///  error(错误描述)
        /// </returns>
        public Hashtable SaveInoutList(bool IsTrans, IList<InoutInfo> orders, IList<InoutDetailInfo> details)
        {
            ...
            try
            {
                ...
                {
                    order.InoutDetailList = new List<InoutDetailInfo>();
                    foreach (var detail in details)
                    {
                        if (detail.order_id == order.order_id)
                        {
                            order.InoutDetailList.Add(detail);
                        }
                    }

                    MSSqlMapper.Instance().Insert("Inout.InsertOrUpdate", order);
                }
        ...
        /// <returns></returns>
        public bool CheckExistInout(InoutInfo order)
        {
            int count = MSSqlMapper.Instance().QueryForObject<int>("Inout.CheckExistInout", order);
        ...
        public bool SetInoutInfo(LoggingSessionInfo loggingSessionInfo, InoutInfo inoutInfo, bool IsTrans, out string strError)
        {
            ...
            try
            {
                ...
                if (inoutInfo.operate.Equals("Create")
                    && (!new BillService().CanHaveBill(loggingSessionInfo, inoutInfo.order_id, out strCount))
                    )
                {
                    //2.提交表单
                    ...
                    //3.更改状态
                    BillModel billInfo = new BillService().GetBillById(loggingSessionInfo, inoutInfo.order_id);
                    if (billInfo != null)
                    {
        ...
        public bool IsExistOrderCode(LoggingSessionInfo loggingSessionInfo, string order_no, string order_id)
        {
            try
            {
                ...
                int n = MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).QueryForObject<int>("Inout.IsExsitOrderCode", _ht);
        ...
        public InoutInfo GetInoutInfoById(LoggingSessionInfo loggingSessionInfo, string orderId)
        {
            try
            {
                ...
                InoutInfo inoutInfo = new InoutInfo();
                inoutInfo = (InoutInfo)MSSqlMapper.Instance().QueryForObject("Inout.SelectById", _ht);
        ...
        public string GetOrderIdByOrderCode(LoggingSessionInfo loggingSessionInfo, string orderNo)
        {
            ...
            string order_id = MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).QueryForObject<string>("Inout.SelectOrderIdByOrderNo", _ht);
            return order_id;
        ...
        public IList<InoutDetailInfo> GetInoutDetailInfoByOrderId(LoggingSessionInfo loggingSessionInfo, string orderId)
        {
            try
            {
                ...
                Hashtable _ht = new Hashtable();
                _ht.Add("OrderId", orderId);
                inoutDetailList = MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).QueryForList<InoutDetailInfo>("InoutDetail.SelectByOrderId", _ht);
        ...
        public IList<InoutInfo> GetInoutListByShiftId(LoggingSessionInfo loggingSessionInfo, string shiftId)
        {
            try
            {
                ...
                return MSSqlMapper.Instance().QueryForList<InoutInfo>("Inout.SelectByShiftId", _ht);
            }
            catch (Exception ex)
            {
                throw (ex);
        ...
        {
            if (IsTrans) MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).BeginTransaction();
            try
            {
                bool bReturn = false;
                InoutInfo inoutInfo = new InoutInfo();
                ...
                if (IsTrans) MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).CommitTransaction();
                return bReturn;
            }
            catch (Exception ex)
            {
                if (IsTrans) MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).RollBackTransaction();
                throw (ex);
            }
        }
        #endregion
