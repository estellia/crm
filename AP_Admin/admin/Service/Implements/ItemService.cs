...
namespace cPos.Admin.Service
{
    ...
    public class ItemService:BaseService
    {
        ...
        {
            return MSSqlMapper.Instance().QueryForList<ItemInfo>("Item.SelectAll", "");
        }
        /// <summary>
        /// 获取单个商品信息
        /// </summary>
        ...
        public ItemInfo GetItemInfoById(LoggingSessionInfo loggingSessionInfo, string item_id)
        {
            ...
            ItemInfo itemInfo = new ItemInfo();
            //获取基础信息
            itemInfo = (ItemInfo)MSSqlMapper.Instance().QueryForObject("Item.SelectById", _ht);
            //获取商品与属性关系信息
        ...
        public string SetItemStatus(LoggingSessionInfo loggingSessionInfo, string Item_Id, string Status)
        {
            ...
            try
            {
                ...
                //提交
                MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).Update("Item.UpdateStatus", itenInfo);
                strResult = "状态修改成功.";
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        public bool SetItemInfo(bool IsTrans, LoggingSessionInfo loggingSessionInfo, ItemInfo itemInfo,out string strError)
        {
            string strResult = string.Empty;
            //事物信息
            ...
            try
            {
                ...
                //5.处理sku
                if (!new SkuService().SetSkuInfo(itemInfo,out strResult))
                {
                    strError = strResult;
                    if (IsTrans) MSSqlMapper.Instance().RollBackTransaction();
                    return false;
        ...
        {
            return SetItemInfo(true, loggingSessionInfo, itemInfo, out strError);
        }
        ...
        public bool IsExistItemCode(LoggingSessionInfo loggingSessionInfo, string item_code, string item_id)
        {
            try
            {
                ...
                int n = MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).QueryForObject<int>("Item.IsExsitItemCode", _ht);
        ...
        public IList<ItemInfo> GetItemListLikeItemName(LoggingSessionInfo loggingSessionInfo, string itemName)
        {
            try
            {
                ...
                return MSSqlMapper.Instance().QueryForList<ItemInfo>("Item.SelectLikeItemName", _ht);
            }
            catch (Exception ex) {
                throw (ex);
            }
        ...
        /// 根据条码查询商品
        /// </summary>
        public IList<ItemInfo> GetItemByBarcode(string barcode)
        {
            try
            {
                Hashtable _ht = new Hashtable();
                _ht.Add("barcode", barcode);
                return MSSqlMapper.Instance().QueryForList<ItemInfo>("Item.GetItemByBarcode", _ht);
            }
            catch (Exception ex)
            {
                throw (ex);
        ...
        {
            return MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).QueryForObject<int>("Item.SelectUnDownloadCount", "");
        }
        /// <summary>
        /// 需要打包的商品信息
        /// </summary>
        ...
        public bool SetItemBatInfo(LoggingSessionInfo loggingSessionInfo, string bat_id, IList<ItemInfo> ItemInfoList,out string strError)
        {
                ...
                itemInfo.ItemInfoList = ItemInfoList;
                MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).Update("Item.UpdateUnDownloadBatId", itemInfo);
                strError = "Success";
                return true;
        }
        /// <summary>
        ...
        public bool SetItemIfFlagInfo(LoggingSessionInfo loggingSessionInfo, string bat_id, out string strError)
        {
                ...
                itemInfo.Modify_User_Id = loggingSessionInfo.CurrentUser.User_Id;
                itemInfo.Modify_Time = GetCurrentDateTime();
                MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).Update("Item.UpdateUnDownloadIfFlag", itemInfo);
                strError = "Success";
                return true;
        }
        #endregion
