...
namespace cPos.Admin.Service
{
    ...
    /// 单位服务
    /// </summary>
    public class UnitService : BaseService, cPos.Admin.Service.Interfaces.IUnitService
    {
        #region 单位
        ...
        public UnitInfo GetUnitById(LoggingSessionInfo loggingSessionInfo, string unitId)
        {
            ...
            unitInfo = (UnitInfo)MSSqlMapper.Instance().QueryForObject("Unit.SelectById", hashTable);
        ...
        public string SetUnitInfo(LoggingSessionInfo loggingSessionInfo, UnitInfo unitInfo)
        {
            ...
            try
            {
                ...
                BillModel billInfo = new BillService().GetBillById(loggingSessionInfo, unitInfo.Id);
...
//#endif
                ////刷新单位级别数据
                //MSSqlMapper.Instance().QueryForObject("Unit.RefreshUnitLevel", null);
        ...
        private bool IsExistUnitCode(LoggingSessionInfo loggingSessionInfo, string UnitCode, string UnitId)
        {
            try
            {
                ...
                int n = MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).QueryForObject<int>("Unit.IsExsitUnitCode", _ht);
        ...
        private bool SetUnitTableInfo(LoggingSessionInfo loggingSessionInfo, UnitInfo unitInfo)
        {
            try
            {
                if (unitInfo != null)
                {
                    ...
                    MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).Update("Unit.InsertOrUpdate", unitInfo);
                }
                return true;
            }
            catch (Exception ex)
        ...
        public IList<UnitInfo> GetRootUnitsByUnitRelationMode(LoggingSessionInfo loggingSession, string unitRelationModeId)
        {
            ...
            return MSSqlMapper.Instance().QueryForList<UnitInfo>("Unit.SelectRootUnits", hashTable);
        }
        ...
        public UnitRelationModeInfo GetDefaultUnitRelationMode(LoggingSessionInfo loggingSession)
        {
            ...
            return (UnitRelationModeInfo)MSSqlMapper.Instance().QueryForObject("UnitRelationMode.SelectDefault", hashTable);
        }
        ...
        public IList<UnitInfo> GetSubUnitsByUnitRelationMode(LoggingSessionInfo loggingSession, string unitRelationModeId, string unitId)
        {
            ...
            return MSSqlMapper.Instance().QueryForList<UnitInfo>("Unit.SelectSubUnitsById", hashTable);
        }
        ...
        private bool SetUnitTableStatus(LoggingSessionInfo loggingSession, string unit_id)
        {
            ...
            {
                //获取要改变的表单信息
                BillModel billInfo = new BillService().GetBillById(loggingSession, unit_id);
                //设置要改变的用户信息
                UnitInfo unitInfo = new UnitInfo();
                unitInfo.Status = billInfo.Status;
                unitInfo.Status_Desc = billInfo.BillStatusDescription;
                ...
                //提交
                MSSqlMapper.Instance(loggingSession.CurrentLoggingManager).Update("Unit.UpdateUnitStatus", unitInfo);
                return true;
            }
            catch (Exception ex)
            {
        ...
        {
            return MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).QueryForObject<int>("Unit.SelectUnDownloadCount", "");
        }
        /// <summary>
        /// 需要打包的Unit信息
        /// </summary>
        ...
        public IList<UnitInfo> GetUnitListPackaged(LoggingSessionInfo loggingSessionInfo, int maxRowCount, int startRowIndex)
        {
            ...
            _ht.Add("EndRow", startRowIndex + maxRowCount);
            _ht.Add("CustomerId", loggingSessionInfo.CurrentLoggingManager.Customer_Id);
            return MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).QueryForList<UnitInfo>("Unit.SelectUnDownload", _ht);
        }
        ...
        public bool SetUnitBatInfo(LoggingSessionInfo loggingSessionInfo, string bat_id, IList<UnitInfo> UnitInfoList)
        {
            ...
            unitInfo.UnitInfoList = UnitInfoList;
            MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).Update("Unit.UpdateUnDownloadBatId", unitInfo);
            return true;
        }
        /// <summary>
        /// 更新Unit表打包标识方法
        ...
        public bool SetUnitIfFlagInfo(LoggingSessionInfo loggingSessionInfo, string bat_id)
        {
            ...
            unitInfo.Modify_User_Id = loggingSessionInfo.CurrentUser.User_Id;
            unitInfo.Modify_Time = GetCurrentDateTime();
            MSSqlMapper.Instance(loggingSessionInfo.CurrentLoggingManager).Update("Unit.UpdateUnDownloadIfFlag", unitInfo);
            return true;
        }
        #endregion
        #endregion
        ...
        public IList<UnitInfo> GetUnitInfoList(Hashtable condition, int maxRowCount, int startRowIndex)
        {
            condition.Add("StartRow", startRowIndex);
            condition.Add("EndRow", startRowIndex + maxRowCount);
            condition.Add("MaxRowCount", maxRowCount);

            IList<UnitInfo> ls= MSSqlMapper.Instance().QueryForList<UnitInfo>("Unit.SearchUnitInfo", condition);
            return ls;
        }

        public int GetUnitInfoListCount(Hashtable condition)
        {
          int count=  MSSqlMapper.Instance().QueryForObject<int>("Unit.SearchUnitInfoCount", condition);
          return count;
        }
