...
function loadSysMenus_callback(result) {
    ...
    if (result != null) {
        for (var i = 0; i < result.length; i++) {
            ...
            subItemCtrl.id = "sysMenu_0_" + subItem.ID;
            subItemCtrl.rootMenuId = subItem.ID;
            sysMenusCtrl.appendChild(subItemCtrl);
            sysMenuIdList.push(subItemCtrl.id);
            var str = "<a href=\"#\" onclick=\"expendSysMenu(true, '" + subItem.ID + 
                "', 0);return false;\">" + subItem.Name + "</a>";
            subItemCtrl.innerHTML = str;
            var subItemLoadCtrl = document.createElement("div");
            subItemLoadCtrl.id = "sysMenu_0_" + subItem.ID + "_L";
...
function expendSysMenu_callback(result, context) {
    ...
    if (result != null) {
        for (var i = 0; i < result.length; i++) {
            ...
            subItemCtrl.id = "sysMenu_" + subLevel + "_" + subItem.ID;
            if (subLevel == 1) {
                subItemCtrl.rootMenuId = subItem.ParentMenuID;
                subItemCtrl.parentMenuId = subItem.ParentMenuID;
            }
            if (subLevel == 2) {
                subItemCtrl.className = "l_2";
                var pEl = document.getElementById("sysMenu_1_" + subItem.ParentMenuID);
            ...
            var str = "";
            var actionScript = "openSysMenu('" + subItem.ID + "', '" + subItem.ParentMenuID + 
                "', '" + subLevel + "', '" + url + "');return false;";
            if (subLevel == 2) str = "<a href=\"" + url + "\" onclick=\"" + actionScript + "\">" + subItem.Name + "</a>";
            else str = "<a href=\"#\" onclick=\"expendSysMenu(true, '" + subItem.ID + 
                    "', '" + subLevel + "');return false;\">" + subItem.Name + "</a>";
            subItemCtrl.innerHTML = str;
            var subItemLoadCtrl = document.createElement("div");
            subItemLoadCtrl.id = "sysMenu_" + subLevel + "_" + subItem.ID + "_L";
            subItemLoadCtrl.innerHTML = "";
            subItemLoadCtrl.style.display = "none";
            ...
            if (menuId2 != undefined && menuId2 != null && menuId2 == subItem.ID)
