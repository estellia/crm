...
*	@OICQ		: 5003427
转载请注明出处 [url]http://www.5aiqu.com[/url]
'**************************************************************************/
...
document.write('<iframe style="position:absolute;width:206px;display:none;z-index:1001; " name="divCalendar" id="divCalendar" frameborder="0" scrolling="no"></iframe>')
...
function Calendar_Show(year, month, day) {
    ...
    if (Calendar_obj2) {
        content += '<li style="width:42px;"><a href="javascript:parent.Calendar_GetDate(' + year + ',' + month + ',0);">当月</a></li>' +
...
function Calendar_GetDate(y, m, d) {
    ...
    if (!Calendar_obj2 || d != 0) {
        if (d > 0) { Calendar_obj.value = y + '-' + m + '-' + d; }
            ...
            var srcElement = objEvent.srcElement;
            if (!srcElement) { srcElement = objEvent.target; }
            var day = srcElement.innerHTML;
            if (day.length == 1) {
                day = "0" + day;
            }
            Calendar_obj.value = y + '-' + m + '-' + day;
        }
        if (Calendar_obj.onchange) {
            Calendar_obj.onchange();
        }
    }
    else {
        if (IsLeapYear(y) && m == 2) { num = MonthDNum[month] + 1; }
        else { num = MonthDNum[month]; }
