...
( function($, undefined) {
        window.touchSlider = function(options) {
            options = options || {};
            var namespace = options.namespace || "touchslider", container = $(options.container);
            ...
            var ret = {
                ...
                stop : stop,
                index : index
            }, isTouchWebkit = "ontouchstart" in window && "WebKitCSSMatrix" in window, touchstart = "touchstart", touchmove = "touchmove", touchend = "touchend", viewport = $(options.viewport, container), scroller = options.scroller ? $(options.scroller, container) : viewport.children(), slides = scroller.children(), pagination = $(options.pagination, container);
            ...
            var switching = ( function() {
                    var inViewport = [0], endCoords = [0], // for calc when an animation
                    toComplete = $.noop,
                    ...
                    return {
                        ...
                        to : function(toIndex, opt) {
                            ...
                            if (indexInViewport !== -1) {
                                ...
                                if (indexInViewport === 0) {
                                    ...
                                    for ( i = 0; i < l; i++) {
                                        slides.eq(inViewport[i]).animate({
                                            left : endCoords[i]
                                        }, {
                                            duration : duration,
                                            queue : false,
                        ...
                        moveStart : function(e) {
                            ...
                            var scrollerLeft = crossLeft(scroller), lastLeft;
                        ...
                        move : function(e, previousPageX) {
                            var diffX = e.pageX - previousPageX, scrollerLeft = crossLeft(scroller), first = slides.eq(inViewport[0]), lastIndex = inViewport.length - 1, last = slides.eq(inViewport[lastIndex]), add, addLeft, deceleration;
                            ...
                            if (((diffX > 0 && scrollerLeft + endCoords[0] + diffX > 0) || (diffX < 0 && scrollerLeft + endCoords[0] > 0)
                            ) && inViewport[0] === 0) {
                            ...
                            if (((diffX > 0 && scrollerLeft + endCoords[lastIndex] < 0) || (diffX < 0 && scrollerLeft + endCoords[lastIndex] + diffX < 0)
                            ) && last.is(slides.last())) {
                        ...
                        moveEnd : function(e, pxInMs, directionX, startTime, distX, distY) {
                            ...
                            var inViewportLength = inViewport.length, scrollerLeft = crossLeft(scroller), toIndex = inViewportLength - 1, opt;
                            if (endCoords[0] + scrollerLeft > 0) {// space in left
                                ...
                                // maximum area
                                var i, right, maximumInViewport = inViewportLength - 1, viewportWidth = viewport.innerWidth();
                                ...
                                if (onFly) {
                                    ...
                                    var touched = inViewportLength - 1, scrollerOffsetLeft = Math.round(scroller.offset().left);
                                    // cast
            ...
            /* Autoplay */
            var mouseInViewport = false, isPlay = false, autoPlayTimeout;
            ...
            function initTouch() {
                var doc = $(document), startTime, defaultPrevented, moving = false, // if mouseup in stopPropogation area
                times, coords, // for accelerate
                startPageX, previousPageX, distX, absDistX, startLeft, startPageY, previousPageY, distY, absDistY, start = function(e) {
                    if (e.which > 1) {
                        return;
                    }
                    ...
                    var i = times.length, pxInMs = 0, directionX = 0;
                    // accelerate
                    while (--i > 0) {
                        if (times[i - 1]) {
