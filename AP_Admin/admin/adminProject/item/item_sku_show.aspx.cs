...
public partial class item_item_sku_show : PageBase
{
    ...
    private void BindRep(string id)
    {
        try
        {
            var source = (new cPos.Admin.Service.SkuService()).GetSkuListByItemId(loggingSessionInfo, id);
    ...
    private void LoadSkuProp()
    {
        try
        {
            var source = new cPos.Admin.Service.SkuPropServer().GetSkuPropList(loggingSessionInfo);
            SkuProInfos = source;
        }
        catch (Exception ex)
        {
