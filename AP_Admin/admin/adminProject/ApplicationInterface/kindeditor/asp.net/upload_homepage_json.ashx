...
using LitJson;
    ...
    public class UploadHomePage : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //文件保存目录路径
            String savePath = aspxUrl+"/attached/";
            //文件保存目录URL
            String saveUrl = aspxUrl + "/attached/";
            if(string.IsNullOrEmpty(host))
                saveUrl = context.Request.Url.Scheme + "://" + context.Request.Url.Authority + "" + aspxUrl + "/attached/";
           ...
           // Loggers.Debug(new DebugLogInfo() { Message = "ProcessRequest" + " hash[\"url\"]: " + host + fileUrl });
            ...
            if (String.IsNullOrEmpty(fileExt) || Array.IndexOf(((String)extTable[dirName]).Split(','), fileExt.Substring(1).ToLower()) == -1)
            {
                ...
                if (!string.IsNullOrEmpty(ftpHostname))
                {
                    ...
                    {
                        //按日期建文件夹
                        string ftpMappingimg = adminProject.ApplicationInterface.upload.FtpMapping.UploadFile(fileInfo, dirName + "/" + ymd, ftpHostname, ftpUser, ftpPass);
                        hash["url"] = ftpMappingimg;
                        //上传缩略图
                        string ftpMappingthumb = adminProject.ApplicationInterface.upload.FtpMapping.UploadFile(fileInfothumb, dirName + "/" + ymd, ftpHostname, ftpUser, ftpPass);
