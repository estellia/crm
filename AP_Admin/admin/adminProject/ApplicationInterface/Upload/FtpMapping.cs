...
namespace adminProject.ApplicationInterface.upload
{
    public class FtpMapping
    {
        ...
        public static string UploadFile(FileInfo fileinfo, string targetDir, string hostname, string username, string password)
        {
            ...
            // targetDir + "/" +
            string URI = "FTP://" + hostname + "/" + targetDir + "/" + target;//"FTP://"+hostname + "/" + targetDir + "/" + target;
            ...
            #endregion
            string ftpMappingUrl = ConfigurationManager.AppSettings["FtpMappingUrl"].ToString();
            return ftpMappingUrl + "/" + targetDir + "/" + fileinfo.Name;  //返回上传图片的mapping地址
        }
