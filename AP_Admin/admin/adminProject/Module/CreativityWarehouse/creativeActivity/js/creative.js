    ...
    var page = {
        ...
        initEvent: function () {
          ...
          that.elems.sectionPage.find(".jsUploadBtn").each(function (i, e) {
            ...
            }).delegate('.setSpread input[type="text"],.setSpread textarea',"blur",function(e){
                debugger;
                   var type=$(this).parents("[data-type]").data("type"),fliedName=$(this).attr("name"),html=$(this).val();
                $(".phoneWebDiv[data-type='"+type+"']").find("[data-view='"+fliedName+"']").html(html);
            }).delegate('.setSpread input[type="text"],.setSpread textarea',"keyup",function(e){
            ...
            that.elems.navigation.delegate("li","click",function(e){
                ...
                var navLiOn=that.elems.navigation.find("li.on").data("showpanel"),  //当前停留的tab
                    ...
                    panelName=$(this).data("showpanel");//当前点击的tab
                      var me=$(this);
                if(panelName=="nav01"&&that.loadData.args.TemplateId){
                ...
                var step=Math.abs($(this).index()-that.elems.navigation.find("li.on").index());
               if( step>1){
                      return false;
               }

                if(navLiOn=="nav02"&&that.themeList.length==0&&panelName=="nav03"){
                    alert("至少添加一种风格");
                    return false;
                }
                if(navLiOn=="nav03"&&that.InteractionList==0&&panelName=="nav04"){
                    alert("至少添加一种活动");
                    return false;
                }
     ...
     /*       that.elems.tabelWrap.delegate(".fontC","click",function(e){
        ...
        loadThemeList:function(){
            ...
            this.loadData.getLEventThemeList(function(data){
                if(data.Data&&data.Data.LEventThemeList&&data.Data.LEventThemeList.length>0){
                    that.themeList=data.Data.LEventThemeList
                }else{
                    that.themeList=[]
                }
        ...
        getLEventInteractionList: function () {
          ...
          this.loadData.getLEventInteractionList(function(data){
              if(data.Data&&data.Data.LEventInteractionList&&data.Data.LEventInteractionList.length>0){
                  that.InteractionList=data.Data.LEventInteractionList
              }else{
       ...
       //设置推广
        setSpread:function() {
          ...
          $(".setSpread").find('[data-tabname]').each(function(){     //3大tab遍历
              ...
              if(type) {
                  if(that.spreadList.length>0){
                      $.each(that.spreadList,function(index,filed){
        ...
        getSpread:function(){
            ...
            this.loadData.operation("", "getSpread", function (data) {
                ...
                that.spreadList=data.Data.SpreadSettingList;
                if(that.spreadList.length==0){
                    return false;
                }
                $('[data-tabname]').each(function(){     //3大tab遍历
                    ...
                    $(this).find('input[type="text"],textarea,[data-view],.jsUploadBtn[data-imgcode]').each(function(){
                        ...
                        switch ($(this).data("view")){
                            ...
                            case "Title": $(this).html(obj["Title"]) ; break;
