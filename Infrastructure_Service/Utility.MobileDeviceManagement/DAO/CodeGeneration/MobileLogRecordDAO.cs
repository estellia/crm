...
namespace JIT.Utility.MobileDeviceManagement.DataAccess
{
    ...
    public partial class MobileLogRecordDAO : BaseMobileDeviceManagementDAO, ICRUDable<MobileLogRecordEntity>, IQueryable<MobileLogRecordEntity>
    {
        ...
        /// 构造函数 
        /// </summary>
        public MobileLogRecordDAO(MobileDeviceManagementUserInfo pUserInfo, ISQLHelper pSQLHelper)
        ...
        public void Create(MobileLogRecordEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
            ...
            strSql.Append("insert into [MobileLogRecord](");
            strSql.Append("[ClientID],[UserID],[LogType],[Message],[Tag],[AppCode],[ResultCode],[LogTime],[IsDelete],[CreateTime],[CreateBy],[LastUpdateBy],[LastUpdateTime])");
            ...
            parameters[2].Value = pEntity.LogType;
            parameters[3].Value = pEntity.Message;
            parameters[4].Value = pEntity.Tag;
            parameters[5].Value = pEntity.AppCode;
            parameters[6].Value = pEntity.ResultCode;
            parameters[7].Value = pEntity.LogTime;
            ...
            pEntity.RecordID = Convert.ToInt32(result);
        }
        ...
        public MobileLogRecordEntity GetByID(object pID)
        {
            //参数检查
            if (pID == null)
                return null;
            ...
            sql.AppendFormat("select * from [MobileLogRecord] where RecordID='{0}' and IsDelete=0 ", id.ToString());
            //读取数据
            MobileLogRecordEntity m = null;
        ...
        /// </summary>
        /// <returns></returns>
        public MobileLogRecordEntity[] GetAll()
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [MobileLogRecord] where isdelete=0");
            //读取数据
            List<MobileLogRecordEntity> list = new List<MobileLogRecordEntity>();
            using (SqlDataReader rdr = this.SQLHelper.ExecuteReader(sql.ToString()))
            {
                while (rdr.Read())
                {
                    MobileLogRecordEntity m;
                    this.Load(rdr, out m);
                    list.Add(m);
                }
            }
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Update(MobileLogRecordEntity pEntity, IDbTransaction pTran, bool pIsUpdateNullField)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
            ...
            //组织参数化SQL
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update [MobileLogRecord] set ");
            ...
            if (pIsUpdateNullField || pEntity.Tag != null)
            ...
            if (pIsUpdateNullField || pEntity.LogTime != null)
            ...
            parameters[10].Value = pEntity.RecordID;
        ...
        /// <param name="pEntity">实体实例</param>
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(MobileLogRecordEntity pEntity, IDbTransaction pTran)
        {
            //参数校验
            if (pEntity == null)
                throw new ArgumentNullException("pEntity");
        ...
        public void Delete(object pID, IDbTransaction pTran)
        {
            ...
            //组织参数化SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MobileLogRecord] set LastUpdateTime=@LastUpdateTime,LastUpdateBy=@LastUpdateBy,IsDelete=1 where RecordID=@RecordID;");
            ...
            { 
                ...
                new SqlParameter{ParameterName="@RecordID",SqlDbType=SqlDbType.VarChar,Value=pID}
            };
            //执行语句
            int result = 0;
            if (pTran != null)
        ...
        /// <param name="pTran">事务实例,可为null,如果为null,则不使用事务来更新</param>
        public void Delete(MobileLogRecordEntity[] pEntities, IDbTransaction pTran)
        {
            //整理主键值
        ...
        /// </summary>
        /// <param name="pEntities">实体实例数组</param>
        public void Delete(MobileLogRecordEntity[] pEntities)
        {
        ...
        public void Delete(object[] pIDs, IDbTransaction pTran)
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.AppendLine("update [MobileLogRecord] set LastUpdateTime='" + DateTime.Now.ToString() + "',LastUpdateBy=" + CurrentUserInfo.UserID + ",IsDelete=1 where RecordID in (" + primaryKeys.ToString().Substring(0, primaryKeys.ToString().Length - 1) + ");");
            //执行语句
            int result = 0;
            if (pTran == null)
        ...
        /// <returns></returns>
        public MobileLogRecordEntity[] Query(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys)
        {
            //组织SQL
            StringBuilder sql = new StringBuilder();
            sql.AppendFormat("select * from [MobileLogRecord] where isdelete=0 ");
        ...
        /// <returns></returns>
        public PagedQueryResult<MobileLogRecordEntity> PagedQuery(IWhereCondition[] pWhereConditions, OrderBy[] pOrderBys, int pPageSize, int pCurrentPageIndex)
        {
            //组织SQL
            ...
            pagedSql.AppendFormat(") as ___rn,* from [MobileLogRecord] where isdelete=0 ");
            //总记录数SQL
            totalCountSql.AppendFormat("select count(1) from [MobileLogRecord] where isdelete=0 ");
            //过滤条件
            if (pWhereConditions != null)
            {
                foreach (var item in pWhereConditions)
            ...
            //执行语句并返回结果
            PagedQueryResult<MobileLogRecordEntity> result = new PagedQueryResult<MobileLogRecordEntity>();
            List<MobileLogRecordEntity> list = new List<MobileLogRecordEntity>();
        ...
        /// </summary>
        /// <returns></returns>
        protected IWhereCondition[] GetWhereConditionByEntity(MobileLogRecordEntity pQueryEntity)
        {
            //获取非空属性数量
            ...
            if (pQueryEntity.Tag != null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "Tag", Value = pQueryEntity.Tag });
            ...
            if (pQueryEntity.LogTime != null)
                lstWhereCondition.Add(new EqualsCondition() { FieldName = "LogTime", Value = pQueryEntity.LogTime });
        ...
        protected void Load(SqlDataReader pReader, out MobileLogRecordEntity pInstance)
        {
            //将所有的数据从SqlDataReader中读取到Entity中
            pInstance = new MobileLogRecordEntity();
            pInstance.PersistenceHandle = new PersistenceHandle();
            pInstance.PersistenceHandle.Load();

            if (pReader["RecordID"] != DBNull.Value)
            {
                pInstance.RecordID = Convert.ToInt32(pReader["RecordID"]);
            }
            ...
            {
                pInstance.LogType = Convert.ToInt32(pReader["LogType"]);
            }
            if (pReader["Message"] != DBNull.Value)
            {
                pInstance.Message = pReader["Message"].ToString();
            }
            if (pReader["Tag"] != DBNull.Value)
            {
                pInstance.Tag = Convert.ToString(pReader["Tag"]);
            }
            ...
            if (pReader["LogTime"] != DBNull.Value)
            {
                pInstance.LogTime = Convert.ToDateTime(pReader["LogTime"]);
            }
